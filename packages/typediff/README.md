...
**Bundler-generated type declarations (tsdown, tsup, rollup-plugin-dts)** — typediff handles `.d.mts` files that import from `.mjs` paths where only the declaration file exists. If you see a warning about exports resolving to `any`, your bundler may be generating an unsupported pattern. Try using `tsc` for declaration generation alongside your bundler for the JS bundle.
